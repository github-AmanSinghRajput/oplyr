...
export function WorkspaceScreen({
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Header */}
      <div>
        <p className="text-xs font-medium text-text-tertiary uppercase tracking-wider mb-1">Workspace</p>
      ...
      <div className="rounded-[var(--radius-panel)] border border-border bg-surface-1 p-5">
        <label className="block mb-3">
          <span className="text-xs font-medium text-text-tertiary uppercase tracking-wider">Project folder</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded-[var(--radius-panel)] border border-border bg-surface-1 p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-text-tertiary uppercase tracking-wider">Connected folder</span>
          </div>
          <p className="text-sm font-medium text-text-primary">{workspace?.projectRoot ?? 'No folder connected yet'}</p>
          <p className="text-xs text-text-tertiary mt-1">Once selected, {activeProviderName} should stay scoped to this folder only.</p>
        </div>

        <div className="rounded-[var(--radius-panel)] border border-border bg-surface-1 p-4">
          <div className="flex items-center gap-2 mb-2">
            <Shield size={14} className="text-warning" />
            <span className="text-xs font-medium text-text-tertiary uppercase tracking-wider">Protected secrets</span>
          </div>
          <p className="text-sm font-medium text-text-primary">
            {workspace?.secretPolicy.length ? workspace.secretPolicy.slice(0, 3).join(', ') : 'Select a folder to see the active policy'}
          </p>
          <p className="text-xs text-text-tertiary mt-1">Secret-like files stay outside normal coding operations.</p>
        </div>

        <div className="rounded-[var(--radius-panel)] border border-border bg-surface-1 p-4">
          <div className="flex items-center gap-2 mb-2">
            <FileCheck size={14} className="text-success" />
            <span className="text-xs font-medium text-text-tertiary uppercase tracking-wider">Change policy</span>
          </div>
          <p className="text-sm font-medium text-text-primary">
            {workspace?.writeAccessEnabled ? 'Approved edits enabled' : 'Chat and advice only'}
          </p>
          <p className="text-xs text-text-tertiary mt-1">No file changes are automatic. Every edit still requires explicit approval.</p>
        </div>
      </div>
      ...
      <div className="rounded-[var(--radius-panel)] border border-danger/30 bg-danger-muted/30 p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <Button variant="destructive" disabled={isResetting} onClick={onResetApp} className="shrink-0">

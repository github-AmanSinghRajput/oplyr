...
> This document is the source of truth for the Oplyr marketing website: layout, copy direction, tone, and section-level design intent. It should not override the product guide or release milestones.
...
- `docs/OPLYR_WEBSITE_BRIEF.md`: website copy and layout truth
...
Alternative: a very subtle audio waveform visualization, gently pulsing as if Oplyr is waiting to hear you speak.
...
**Product visual:** A clean screenshot or short recording of the Oplyr voice session screen with the waveform active, live transcript visible, and the app responding in real time. If a video/GIF is available, use that instead.

**Feature cards (grid of 4):**

**Card 1: Voice-native**
```
Speak naturally. Oplyr transcribes, understands context,
...
**Card 3: Local-first**
```
Oplyr runs on your Mac. Your code never leaves your machine.
...
Oplyr wraps those subscriptions directly.
...
Oplyr guides you through setup on first launch.
```
...
## Section 6: What Makes Oplyr Different
...
**Left column copy:**
```
Oplyr is not a plugin for your IDE. It's not a browser tab you
...
Oplyr gives you a spoken engineering partner that operates inside
...
**Subheadline:**
```
Oplyr is just getting started. Here's a taste of what's next.
```
...
**Then — 1.0**
```
The first serious public Oplyr launch.
...
Oplyr starts in advisory mode. File changes require you
...
to improve Oplyr. We never track, log, or analyze
the content of your code or conversations.

[check] Secret file protection
Oplyr blocks access to sensitive files such as .env,
...
Download Oplyr for macOS
[Download DMG button]

Version 1.0.0-beta · macOS 14+ · Apple Silicon
```
...
GitHub (if public)
Twitter / X
Contact: hello@oplyr.com (or whatever domain you get)
```

**Bottom line:**
```
Made with voice, code, and an unreasonable amount of coffee.
2026 Oplyr. All rights reserved.
```
...
**Meta description:**
```
Talk to your AI coding agent naturally. Review every file change before it happens. Oplyr is a local-first macOS app that wraps your existing AI subscription into a voice-first engineering workflow. Invite-only beta.
```
...
PRODUCT:   Meet Oplyr. Your voice-first coding operator.
...
BETA:      Oplyr is in invite-only beta.
